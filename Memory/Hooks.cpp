...
__int64 __fastcall Hooks::renderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx)
{
...

	{
		...
		static std::string horionStr = std::string("Horion Dank Edition");					 // Static Horion logo / text
		static float       horionStrWidth = DrawUtils::getTextWidth(&horionStr); // Graphical Width of Horion logo / text
		static std::string dlStr = std::string("Weed Good!");					 // Static Horion logo / text
		...
		// Draw Horion logo
		{
			DrawUtils::drawText(vec2_t(windowSize.x - 400,windowSize.y - 370), &horionStr, nullptr, 1.5f);
			DrawUtils::drawText(vec2_t(windowSize.x - 375.1f,windowSize.y - 350.75f), &dlStr, nullptr, 1.3f);
		}
